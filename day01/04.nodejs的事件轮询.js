/* 
    nodeJS的事件轮询：
        ...
        4.宏任务也是根据异步代码不同，而产生多种回调队列，nodejs会依次轮询这几个回调队列：timers、pendding callbacks、idle、poll、check、close callbacks
    ...
    宏任务的轮询顺序
        ...
        4.poll阶段：处理I/O或者网络请求等异步操作的回调函数
            ...
            - 当poll阶段为空，会一直等待poll中有其他的回调函数
                - 但是当 timer阶段的计时器到期了，或者check阶段有setImmediate等待执行的时候，会直接跳入check阶段
